...
TODO :
1. using char* or bytes arr in binkeys
2. change using yield to return value progressive
3. using c struct as result for one entry
...
cdef class GbbqReader(object):
    ...
    def get_df(self, fname):
        ...
        cdef char* encrypt_data
        cdef unsigned int data_offset
        cdef unsigned int eax, ebx, num, numold
        cdef unsigned int eax_add, eax_xor
        ...
        if f != NULL:
            ...
            encrypt_data = <char*>PyMem_Malloc(1 * filesize)
            fread(encrypt_data, 1, filesize, f)
            fclose(f)

            pos = 0
            (count, ) = struct.unpack("<I", encrypt_data[pos: pos+4])
            pos += 4
            data_offset = pos
